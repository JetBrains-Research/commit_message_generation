...
@pytest.fixture
def default_setting():
    model = EncoderDecoderModule(diff_tokenizer=AutoTokenizer.from_pretrained("distilbert-base-uncased"),
...
def test_with_and_without_prefix(default_setting, context, prefix, expected):
    ...
    tokenized_context = model._diff_tokenizer(context, return_tensors="pt").input_ids
    tokenized_context_w_prefix = model._msg_tokenizer(context + prefix, return_tensors="pt").input_ids

    min_len = 5
    max_len = 5

    results_with_prefix_fn = model.generate({"msg_input_ids": tokenized_context,
    ...
    sequences_with_prefix_fn = model._msg_tokenizer.batch_decode(results_with_prefix_fn[:, tokenized_context.shape[1] :])

    results_without_prefix_fn = model.generate({"msg_input_ids": tokenized_context_w_prefix,
