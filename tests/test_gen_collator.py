import pytest
from src.data_utils import DataCollator
...
def test_without_history(msgs, diffs, tokenizers):
    ...
    data_collator = DataCollator(
        context_ratio=0.1, src_tokenizer=encoder_tok, trg_tokenizer=decoder_tok, max_len=200, generation=True, with_history=False
    )
    res = data_collator(inputs)
