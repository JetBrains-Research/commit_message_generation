...
for i, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    if i == 1252844:  # first example: considering previous iteration as warm-up
        # wait for warm-up to finish
        torch.cuda.synchronize()
        # run operations one more time to get more accurate values
        res = generate(model=model,
                       cfg=cfg,
                       data_processor=data_processor,
                       diff=row["diff"],
