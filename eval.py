    ...
    if "ckpt_path" in cfg:
        ...
        model = CMCModule.load_from_checkpoint(
            ...
            **cfg.model,
            ...
            preds_table_name=f"context-ratio_{cfg.dataset.context_ratio}_{('with-history' if cfg.dataset.generate_with_history else 'without-history')}",
        )
    else:
        ...
        model = CMCModule(
            **cfg.model,
            ...
            preds_artifact_name=f"{cfg.model_name}_preds",
            preds_artifact_type="multilang preds",
            preds_table_name=f"context-ratio_{cfg.context_ratxio}_{('with-history' if cfg.dataset.generate_with_history else 'without-history')}",
        )
