...
class AccuracyCalculation:
    def __init__(self, model: EncoderDecoder, max_len: int, greedy: bool, config: Config, logger) -> None:
        super().__init__()
        self.model = model
    ...
    def conduct(self, dataset: Dataset, dataset_label: str) -> List[List[List[str]]]:
        self.logger.info(f'Start conducting accuracy calculation experiment for {dataset_label}...')
        data_iterator = DataLoader(dataset, batch_size=self.batch_size)
        correct_all_k, total, max_top_k_decoded = \
            calculate_top_k_accuracy(self.topk_values, data_iterator, self.decode_method, self.logger)
        for correct_top_k, k in zip(correct_all_k, self.topk_values):
            self.logger.info(f'Top-{k} accuracy: {correct_top_k} / {total} = {correct_top_k / total}')
