...
class DataCollatorWithHistory:
    ...
    - msg_input_ids, msg_attention_mask, msg_labels: history + current message
        - <bos> history_1 \n ... history_k \n cur_msg \n
    """
    ...
    def __call__(
        ...
        if not self.testing:
            ...
            for message_ids, history_ids in zip(message_inputs, history_inputs):
                ...
                cur_labels = [message_ids[: self.max_len - self.max_len - 1 - len(self.sep_tokens)]]
                cur_len = (
                    len(message_ids[: self.max_len - self.max_len - 1 - len(self.sep_tokens)])
                    + 1
                    + len(self.sep_tokens)
                )
                ...
                for history_input_ids in history_ids[::-1]:
                    ...
                    cur_ids.insert(1, history_input_ids + self.sep_tokens)
                    cur_labels.insert(1, [-100 for _ in history_input_ids + self.sep_tokens])
            ...
            for i, (diff_id_tensor, diff_mask_tensor, msg_id_tensor, msg_mask_tensor, labels_tensor) in enumerate(
                zip(all_diff_ids, all_diff_masks, all_msg_ids, all_msg_masks, all_msg_labels)
            ):
                # pad ids with pad_token_id (which doesn't really matter for GPT-2)
                all_msg_ids[i] = torch.nn.functional.pad(
                    msg_id_tensor,
                    pad=[0, msg_max_len - msg_id_tensor.numel()],
                    mode="constant",
                ...
                all_msg_labels[i] = torch.nn.functional.pad(
                    labels_tensor, pad=[0, msg_max_len - labels_tensor.numel()], mode="constant", value=-100
                )

                # pad masks with zeros
                all_msg_masks[i] = torch.nn.functional.pad(
                    msg_mask_tensor, pad=[0, msg_max_len - msg_mask_tensor.numel()], mode="constant", value=0
                )
...
@dataclass
class DataCollatorWithHistoryGeneration:
    """
    Data collator used to
    1) collate author's history with current message
    2) pad and concatenate everything into tensor
    ...
    - msg_input_ids, msg_attention_mask, msg_labels: history + current message
     ...
     - generation_input_ids, generation_attention_mask: history, used as context/prompt for generation
        - format when history is not empty: <bos> history_1 \n ... history_k \n
        - format when history is empty: <bos>
    """
    ...
    def __call__(
        ...
        for message_ids, history_ids in zip(message_inputs, history_inputs):
            ...
            cur_labels = [message_ids[: self.max_len - self.max_len - 1 - len(self.sep_tokens)]]
            cur_len = (
                len(message_ids[: self.max_len - self.max_len - 1 - len(self.sep_tokens)]) + 1 + len(self.sep_tokens)
            )
            ...
            for history_input_ids in history_ids[::-1]:
                ...
                cur_ids.insert(1, history_input_ids + self.sep_tokens)
                cur_generation_ids.insert(1, history_input_ids + self.sep_tokens)
                cur_labels.insert(1, [-100 for _ in history_input_ids + self.sep_tokens])

            # flatten everything into one sequence and convert to tensor of torch.int64
            cur_ids = torch.tensor([ex for sublist in cur_ids for ex in sublist], dtype=torch.int64)
            cur_generation_ids = torch.tensor(
                [ex for sublist in cur_generation_ids for ex in sublist], dtype=torch.int64
            )
            cur_labels = torch.tensor([ex for sublist in cur_labels for ex in sublist], dtype=torch.int64)
        ...
        for i, (
            ...
            all_generation_masks[i] = torch.nn.functional.pad(
                gen_mask_tensor, pad=[gen_max_len - gen_mask_tensor.numel(), 0], mode="constant", value=0
            )
...
class DataCollatorWithoutHistory:
    ...
    def __call__(self, examples: List[Dict[str, List[List[int]]]]) -> Dict[str, torch.Tensor]:
        if not self.testing:
            diff_inputs = [e["diff_input_ids"] for e in examples]  # 2D - list of lists
            message_inputs = [e["msg_input_ids"] for e in examples]  # 2D - list of lists
