...
class DataCollatorWithHistory:
    ...
    - msg_input_ids, msg_attention_mask, msg_labels: used for training and completion metrics
        - <bos> history_1 \n ... history_k \n cur_msg <eos> (history + current message)
    - generation_input_ids, generation_attention_mask: used for generation metrics
        - <bos>                                             (if history is empty)
...
class DataCollatorWithoutHistory:
    """
    Data collator used to
    1) just pad and concatenate everything into tensor, no history in this version!
    - diff_input_ids, diff_attention_mask: simply diffs, used for everything
    - msg_input_ids, msg_attention_mask, msg_labels: current message, used for training and completion metrics
        - <bos> cur_msg <eos>
    - generation_input_ids, generation_attention_mask: used for generation metrics
        - <bos>
    """
    ...
    def __call__(
        ...
        for message_ids in message_inputs:
            cur_ids = (
                [self.trg_tokenizer.bos_token_id] + message_ids[: self.max_len - 2] + [self.trg_tokenizer.eos_token_id]
            )
            cur_labels = [-100] + message_ids[: self.max_len - 2] + [-100]
            cur_generation_ids = [self.trg_tokenizer.bos_token_id]

            # flatten everything into one sequence and convert to tensor of torch.int64
            cur_ids = torch.tensor(cur_ids, dtype=torch.int64)
            cur_generation_ids = torch.tensor(cur_generation_ids, dtype=torch.int64)
            cur_labels = torch.tensor(cur_labels, dtype=torch.int64)
