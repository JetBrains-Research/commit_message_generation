...
class DataPreprocessor:
    ...
    def create_files(ds_root_path: str):
        for part in ["train", "val", "test"]:
            ...
            diff_input_ids = DataPreprocessor.tokenize_diffs(
                df["diff"].tolist(), RobertaTokenizerFast.from_pretrained("microsoft/codebert-base")
            )
            print("Tokenizing msgs")
            msg_input_ids = DataPreprocessor.tokenize_messages(
                df["message"].tolist(), GPT2TokenizerFast.from_pretrained("distilgpt2")
            )
            ...
            if part != "train":
                ...
                df[["diff_input_ids", "pos_in_history", "author"]].to_json(
                    os.path.join(ds_root_path, f"{part}.json"), lines=True, orient="records"
                )
            else:
                with open(os.path.join(ds_root_path, f"{part}.json"), "w") as outfile:
                    for diff_line, position_line, author_line in tqdm(
                        zip(diff_input_ids, df["pos_in_history"].tolist(), df["author"].tolist())
                    ):
                        json.dump(
                            {"diff_input_ids": diff_line, "pos_in_history": position_line, "author": author_line},
                            outfile,
                        )
                        outfile.write("\n")
